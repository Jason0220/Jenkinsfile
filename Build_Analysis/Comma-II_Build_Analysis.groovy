            ...
            steps {
                dir ('target/apollo_4b_evb') {
                    sh """
                        ...
                        git reset --hard
                        git clean -fxd
                        build-wrapper-linux-x86-64 --out-dir ../../bw-output_sku1 scons -j\$(nproc)
                    """
                }
            }
        }
            ...
            steps {
                withSonarQubeEnv('SonarQube_46') {
                    script {
                        ...
                        def sonarExclusions = """sonar.exclusions=
                            drivers/ppg/pah8151/factory_test/pah_factory_test_v3/**/*,
                            ...
                            **/*.php,
                            **/*.yaml
                        """.replaceAll(/\s+/, ' ').trim()  // 去除换行和多余空格，转为单行

                        def sonarCpdExclusions = """sonar.cpd.exclusions=
                        ...
                        sh """
                            ...
                            echo "sonar.projectKey=Comma-II" >> sonar-project.properties
                            echo "${sonarExclusions}" >> sonar-project.properties
                            echo "${sonarCpdExclusions}" >> sonar-project.properties
                        """

                        sh """
                            sonar-scanner \
                                -Dsonar.projectKey=Comma-II \
                                -Dsonar.branch.name=${env.GERRIT_BRANCH}   \
                                -Dsonar.projectName="Comma-II_SKU1" \
                                -Dsonar.projectVersion="${version}" \
                                -Dsonar.sources=. \
                                -Dsonar.cfamily.build-wrapper-output=./bw-output_sku1 \
                                -Dsonar.host.url=http://10.10.192.46:9000/sonarqube \
                                -Dsonar.login=sqp_80c3c15cb958cae599520b696bc4e429178034ba
                        """
                    }
                }
            }
                ...
                withSonarQubeEnv('SonarQube_46') {
                    script {
                        ...
                        sh """
                            ...
                            echo "sonar.projectKey=Comma-II_UA" >> sonar-project.properties
                            ...
                            sonar-scanner \
                                -Dsonar.projectKey=Comma-II_UA \
                                -Dsonar.branch.name=${env.GERRIT_BRANCH}   \
                                -Dsonar.projectName="Comma-II_UA" \
                                -Dsonar.projectVersion="${version}" \
                                -Dsonar.sources=. \
                                -Dsonar.cfamily.build-wrapper-output=./bw-output_ua \
                                -Dsonar.host.url=http://10.10.192.46:9000/sonarqube \
                                -Dsonar.login=sqp_5472152e8027d1a24189188f6848515e9e128fce
                        """
                    }
                }
            }
