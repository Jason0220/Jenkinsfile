pipeline {
    ...
    environment {
        ...
        TARGET_ZIP_NAME = "rubis_fw_dailybuild_${DAILY_VERSION}.zip"
    }
    ...
    stages {
        ...
        stage('BUILD_USB_FW') {
            steps {
                script {
                    def configFile = "${env.WORKSPACE}/tools/rubis_config.sh"
            ...
            steps {
                script {
                    ...
                    sh """
                        ...
                        cp ${env.NOTES_FILE} out/release/${env.NOTES_FILE}
                    """
                    echo "Release notes generated: out/release/${env.NOTES_FILE}"
                }
            }
        }
        ...
        stage('BACKUP_to_FTP') {
            steps {
                script {
                    def url = "2025 Spinel_Rubis/dailyBuild/rubis"
                                ...
                                [
                                    ...
                                    transfers: [
                                        [
                                            ...
                                            sourceFiles: "out/release/rubis_fw_dailybuild_${env.DAILY_VERSION}.zip"
                                        ]
                                    ],
        ...
        stage('Archiving') {
            steps {
                script {
                    ...
                    def subject = "[Rubis Daily Build] archived successfully - ${env.TARGET_ZIP_NAME}"
                    def body = """
