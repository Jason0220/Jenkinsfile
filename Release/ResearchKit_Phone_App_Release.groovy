pipeline {
    ...
    stages {
        ...
        stage('BUILD') {
            steps {
                script {
                    ...
                    def buildCommand = ""
                    if (env.sku == 'Generic') {
                        buildCommand = './gradlew assembleresearchkitRelease'
                    } else if (env.sku == 'Dynaudio') {
                        buildCommand = './gradlew assembleresearchkitRelease -Penv=danna'
                    } 
        ...
        stage('BACKUP_to_Artifactory') {
            steps {
                script {
                    def outDir = "GTBluetoothTools/app/build/outputs/apk/researchkit/release"

                    withEnv([
                        "outDir=${outDir}"
                    ]) {
                        sh """
                            url="http://10.10.192.15:8081/artifactory/ResearchKit_Phone_App/Release/${env.sku}/${env.verName}.\$(date +%Y%m%d%H%M)"
                            echo \$url
        ...
        stage('BACKUP_FW_to_FTP') {
            steps {
                script {
                    ...
                    def url = "2025 ResearchKit/Release/PhoneApp/${env.sku}/${env.sku}_PhoneApp_${env.verName}.${currentDate}/"
                    def outDir = "GTBluetoothTools/app/build/outputs/apk/researchkit/release/"
                ...
                script {
                    ...
                    withEnv([
                            ...
                            script: """
                                ...
                                for file in \$filePath; do
                                    if [ -f "\$file" ]; then
                                        ...
                                        -F "verName=\$VER_NAME-${env.sku}" -F "deviceTypeId=15" -F "extType=.apk" \
                                        -F "md5=\$MD5_VAL" "\$url"
                                        if [ \$? -ne 0 ]; then
                                            echo "Failed to upload \$file"
                                            exit 1
                        ...
                        if (output.length == 6) {
                            ...
                            emailext(
                                subject: "ResearchKit_Phone_App of ${env.sku} ${EXPORT_FILE_NAME} Release",
                                body: "Dear all, <br><br>we're pleased to announce the release of ResearchKit_Phone_App of ${env.sku} ${EXPORT_FILE_NAME}. <br>Download from https://alpha.goertek.com:8883/api/firmware/download?md5=${EXPORT_MD5}. <br>Feedback is welcome at jeremy.li@goertek.com",
                    ...
                    def message = """
                    {
                        "msg_type": "text",
                        "content": {
                            "text": "ResearchKit Phone App of ${env.sku} ${env.verName} is available, please downoad it from the following link: https://alpha.goertek.com:8883/api/firmware/download?md5=${EXPORT_MD5}"
                        }
                    }
                    """
