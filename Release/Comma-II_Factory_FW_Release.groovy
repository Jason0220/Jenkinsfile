            ...
            steps {
                script {
                    ...
                    if (params.SKU == 'sku1') {
                        ...
                        error "Unsupported SKU value: ${params.SKU}. Supported values are 'sku1' or 'sku2'."
                    }
                        ...
                        withEnv([
                            ...
                            sh '''
                                ...
                                if [ "$OTA" = "Full" ]; then
                                    ...
                                    python ota_packager.py type=factory_"$SKU" version="$APP_VERSION"
                                else
                                    echo "Generating App OTA upgrade package"
                                    python package_single_firmware.py factory_"$SKU" version="$APP_VERSION"
                                fi
                            '''
                        }
                    }
        ...
        stage('Prepare_OTA_bin') {
            steps {
                dir("${env.otaPath}") {
                    script {
                        // 1. 查找升级文件和目录
                        def upgradePath = sh(script: 'ls -d upgrade_[af]*/ 2>/dev/null || true', returnStdout: true).trim()
                        ...
                        // 2. 构造新文件名
                        def newBaseName = "commaii_${params.SKU}_factory_${params.APP_VERSION}"
                ...
                dir("${env.WORKSPACE}") {
                    script {
                        ...
                        sh """
                            ...
                            cd out
                            zip -r "commaii_${params.SKU}_factory_${params.APP_VERSION}.\$(date +%Y%m%d%H%M).zip" * || { echo "Zip failed"; exit 1; }
                        """
            ...
            steps {
                script {
                    withEnv([
                        ...
                        sh """
                            url="http://10.10.192.15:8081/artifactory/comma-ii-\${SKU}_factory/Release/commaii-\${SKU}_factory_\$APP_VERSION.\$(date +%Y%m%d%H%M)"
                            echo \$url
        ...
        stage('BACKUP_to_FTP') {
            steps {
                script {
                    ...
                    def url = "2024 Comma-II/Release/${params.SKU}_factory/commaii_${params.SKU}_factory_${params.APP_VERSION}.${currentDate}"
                                ...
                                [
                                    ...
                                    transfers: [
                                        ...
                                        [
                                            ...
                                            sourceFiles: "${env.otaPath}/${env.upgradePath}${env.otaFileName}"
                                        ]
                                    ],
            ...
            steps {
                script {
                    ...
                    if (output.length == 6) {
                        ...
                        emailext(
                            subject: "${EXPORT_FILE_NAME} Comma-II ${params.SKU} Factory FW Release",
                            body: "Dear all, <br><br>We're pleased to announce the release of ${EXPORT_FILE_NAME} for ${params.SKU} factory variant. <br>Download from https://alpha.goertek.com:8883/api/firmware/download?md5=${EXPORT_MD5}. <br>Feedback is welcomed at jeremy.li@goertek.com",
                    ...
                    def message = """
                    {
                        "msg_type": "text",
                        "content": {
                            "text": "Comma-II ${env.sku} factory firmware release ${env.verName} for factory variant is available, please downoad it from the following link: https://alpha.goertek.com:8883/api/firmware/download?md5=${EXPORT_MD5}"
                        }
                    }
                    """
