pipeline {
    ...
    stages {
            ...
            steps {
                checkout([
                    ...
                    userRemoteConfigs: [[
                        ...
                        url: 'ssh://10.10.192.13:29418/EMG_PhoneApp'
                    ]]
                ])
            }
        }
        ...
        stage('BUILD') {
            steps {
                script {
                    ...
                    echo "Insert domestic mirrors into build.gradle..."
                    sh '''
                        sed -i 's|repositories {|repositories {\\n    maven { url "https://maven.aliyun.com/repository/google" }\\n    maven { url "https://maven.aliyun.com/repository/jcenter" }\\n    maven { url "https://maven.aliyun.com/repository/maven-central" }|' build.gradle
                    '''
                    sh '''
                        sed -i '/allprojects {/,/}/ s|repositories {|repositories {\\n    maven { url "https://maven.aliyun.com/repository/google" }\\n    maven { url "https://maven.aliyun.com/repository/jcenter" }\\n    maven { url "https://maven.aliyun.com/repository/maven-central" }|' build.gradle
                    '''
                    echo "Domestic mirror insertion completed. Current content of build.gradle:"
                    sh 'cat build.gradle'

                    def versionFile = 'versions.gradle'
        ...
        stage('BACKUP_to_Artifactory') {
            steps {
                script {
                    def outDir = "app/build/outputs/apk/release"

                    withEnv([
                        "outDir=${outDir}"
                    ]) {
                        sh """
                            url="http://10.10.192.15:8081/artifactory/EMG_PhoneApp/Release/${env.verName}.\$(date +%Y%m%d%H%M)"
                            echo \$url
        ...
        stage('BACKUP_FW_to_FTP') {
            steps {
                script {
                    ...
                    def url = "2025 EMG_PhoneApp/Release/EMG_PhoneApp_${env.verName}.${currentDate}/"
                    def outDir = "app/build/outputs/apk/release/"
                ...
                script {
                    ...
                    withEnv([
                            ...
                            script: """
                                ...
                                for file in \$filePath; do
                                    if [ -f "\$file" ]; then
                                        ...
                                        -F "verName=\$VER_NAME" -F "deviceTypeId=3" -F "extType=.apk" \
                                        -F "md5=\$MD5_VAL" "\$url"
                                        if [ \$? -ne 0 ]; then
                                            echo "Failed to upload \$file"
                                            exit 1
                        ...
                        if (output.length == 6) {
                            ...
                            emailext(
                                subject: "${EXPORT_FILE_NAME} EMG_PhoneApp Release",
                                body: "Dear all, <br><br>we're pleased to announce the release of EMG_PhoneApp ${EXPORT_FILE_NAME}. <br>Download from https://alpha.goertek.com:8883/api/firmware/download?md5=${EXPORT_MD5}. <br>Feedback is welcome at jeremy.li@goertek.com",
                    ...
                    def message = """
                    {
                        "msg_type": "text",
                        "content": {
                            "text": "EMG Phone App ${env.verName} is available, please downoad it from the following link: https://alpha.goertek.com:8883/api/firmware/download?md5=${EXPORT_MD5}"
                        }
                    }
                    """
