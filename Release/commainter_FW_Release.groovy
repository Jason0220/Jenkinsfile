pipeline {
    ...
    stages {
        ...
        stage('CODE_SYNC') {
            ...
            steps {
                checkout scmGit(branches: [[name: '*/commainter']], 
                extensions: [], 
                userRemoteConfigs: [[credentialsId: 'Jenkins', 
                name: 'origin', 
                refspec: '+refs/heads/commainter:refs/remotes/origin/commainter', 
        ...
        stage('PREPARE_USB_DEBUG_FW_for_PACKAGING') {
            steps {
                script {
                   sh """
                        pwd
            ...
            steps {
                dir("${env.otaPath}") {
                    script {
                        ...
                        // 2. 构造新文件名
                        def newBaseName = "commainter_${params.APP_VERSION}"
                ...
                dir("${env.WORKSPACE}") {
                    script {
                        ...
                        sh """
                            pwd
                            mkdir -p out/App_release out/Bootloader out/OTA_release
                            cp "${env.targetDir}"/commaii_ap.* ./out/App_release/ || { echo "Failed to copy commaii_ap.*"; exit 1; }
                            cp "${env.otaPath}"/firmware/*.* ./out/Bootloader/ || { echo "Failed to copy firmware"; exit 1; }
                            cp "$otaFilePath" ./out/OTA_release/ || { echo "Failed to copy OTA file"; exit 1; }
                            
                            cd out
                            zip -r "commainter_${params.APP_VERSION}.\$(date +%Y%m%d%H%M).zip" * || { echo "Zip failed"; exit 1; }
                        """
                        
                        // 获取生成的 ZIP 文件名
                        env.ZIP_FILE_NAME = sh(script: 'ls out/commainter_*.zip | head -n 1', returnStdout: true).trim()
            ...
            steps {
                script {
                    withEnv([
                        ...
                        sh """
                            url="http://10.10.192.15:8081/artifactory/commainter/Release/\$APP_VERSION.\$(date +%Y%m%d%H%M)"
                            echo \$url
                            ...
                            # 上传 OTA_release commaii_ap.* 文件
                            for file in ./out/App_release/commaii_ap.*; do
                                if [ -f "\$file" ]; then
                                    curl -u "\$userpass" -T "\$file" "\$url/App_release/"
                                    if [ \$? -ne 0 ]; then
                                        echo "Failed to upload file \$file"
                                        exit 1
                                    fi
                            ...
                            if [ -f "\$file" ]; then
                                curl -u "\$userpass" -T "\$file" "\$url/OTA_release/"
                                if [ \$? -ne 0 ]; then
                                    echo "Failed to upload file \$file"
                                    exit 1
                                fi
            ...
            steps {
                script {
                        ...
                        script: """
                            ...
                            for file in \$filePath; do
                                if [ -f "\$file" ]; then
                                    ...
                                    -F "verName=${env.verName}" -F "deviceTypeId=17" -F "extType=.zip" \
                                    -F "md5=\$MD5_VAL" "\$url"
                                    if [ \$? -ne 0 ]; then
                    ...
                    if (output.length == 6) {
                        ...
                        emailext(
                            subject: "${EXPORT_FILE_NAME} commainter FW Release",
                            body: "Dear all, <br><br>We're pleased to announce the release of ${EXPORT_FILE_NAME} for commainter. <br>Download from http://alpha.goertek.com:4433/api/firmware/download?md5=${EXPORT_MD5}. <br>Feedback is welcomed at jeremy.li@goertek.com",
                    ...
                    def message = """
                    {
                        "msg_type": "text",
                        "content": {
                            "text": "commainter firmware release ${env.verName} is available, please downoad it from the following link: http://alpha.goertek.com:4433/api/firmware/download?md5=${EXPORT_MD5}"
                        }
                    }
                    """
